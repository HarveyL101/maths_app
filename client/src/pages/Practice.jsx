import { Accordion, Footer, SearchBar, TitleBar } from '../utils/index.js';
import { curriculum } from '../components/Questions/curriculumConfig.js';
import { useState } from 'react';
import { useNavigate } from 'react-router-dom';

// Max Quiz size variable
const MAX_QUIZ_SIZE = 10;

const Practice = () => {
  const [questions, setQuestions] = useState([]);

  const navigate = useNavigate();
  const token = localStorage.getItem("jwt");

  // Fetch Relevant Questions
  const fetchQuestions = async (params) => {
    try {
      const query = new URLSearchParams(params).toString();

      const res = await fetch(`/api/questions?${query}`, {
        headers: { Authorization: `Bearer ${token}` }
      });

      const data = await res.json();
      setQuestions(data);
    } catch(error) {
      console.error("Failed to fetch questions", error);
    }
  };

  // Groups fetched questions into pools split via subtopic
  const createQuestionPool = (data) => {
    const map = {};

    data.forEach((q) => {
      if (!map[q.subtopic_id]) {
        map[q.subtopic_id] = {
          subtopic_id: q.subtopic_id,
          subtopic_name: q.subtopic_name,
          topic_name: q.topic_name,
          year_group: q.year_group,
          creator_surname: q.creator_surname,
          questions: [],
        };
      }
      map[q.subtopic_id].questions.push(q);
    });
    return Object.values(map);
  };

  const fetchProgress = async () => {
    const token = localStorage.getItem("jwt");

    const res = await fetch('/api/progress', {
      headers: {
        Authorization: `Bearer ${token}`
      }
    });

    if (!res.ok) throw new Error("Failed to fetch progress");

    return await res.json();
  };

  // A PRACTICE version of the startQuizHandler found in Learn.jsx
  // Only features questions that have been previously completed.
  const startPracticeQuiz = async (poolEntry) => {
    try {
      const progress = await fetchProgress();

      const completed = progress
        .filter(p => p.is_correct)
        .map(p => p.question_id);

      const wrong = progress
        .filter(p => !p.is_correct)
        .map(p => p.question_id);

      let filtered = [...poolEntry.questions];

      // Priority 1: Wrong questions
      const weak = filtered.filter(q => wrong.includes(q.question_id));
      // Priority 2: Unanswered questions
      const unanswered = filtered.filter(q => !completed.includes(q.question_id));

      // Ordered in priority: Weak -> Unanswered -> Rest of pool
      filtered = weak.length > 0 
        ? weak 
        : unanswered.length > 0 
        ? unanswered 
        : filtered;

      // Shuffle and portion the quiz questions to MAX_QUIZ_SIZE
      const shuffled = [...filtered].sort(() => Math.random() - 0.5);
      const slice = shuffled.slice(0, MAX_QUIZ_SIZE);

      // Move data and user into the Quiz screen to complete questions
      navigate(`/quiz/${poolEntry.subtopic_id}`, {
        state: {
          questions: slice,
          subtopicName: poolEntry.subtopic_name,
          topic: poolEntry.topic_name,
          year: poolEntry.year_group,
          creatorSurname: poolEntry.creator_surname,
          returnTo: '/practice'
        }
      });
    } catch (error) {
      console.error("Error starting practice quiz: ", error);
      alert("Failed to start practice quiz, please try again later.");
    }
  };

  const buildAccordionData = (curriculum) => {
    return Object.entries(curriculum).map(([year, topics]) => ({
      title: `Year ${year}`,
      children: Object.entries(topics).map(([topic, subtopics]) => ({
        title: topic,
        children: subtopics.map((subtopic) => ({
          title: subtopic,
          year, topic, subtopic
        }))
      }))
    }));
  };
  
  const accordionData = buildAccordionData(curriculum);
  const pools = createQuestionPool(questions);

  // RENDER THE POOLS
  const renderPools = (pools) => {
    return pools.map((pool) => {
      const count = Math.min(pool.questions.length, MAX_QUIZ_SIZE);

      return (
        <div key={pool.subtopic_id} className='creator-card'>
          <div className='creator-card-body'>
            <p className='creator-card-title'>{pool.subtopic_name}</p>
            <p className='creator-card-meta'>{pool.creator_surname}</p>
          </div>

          <div className='creator-card-footer'>
            <div className='creator-tag creator-tag--year'>
              Year {pool.year_group}
            </div>
            <div className='creator-tag creator-tag--topic'>
              {pool.topic_name}
            </div>
          </div>

          <button className='creator-card-button' onClick={() => startPracticeQuiz(pool)}>
            {`Practice (${count}) ->`}
          </button>
        </div>
      );
    })
  };

  // USER INTERFACE
  return (
    <div className='page-wrapper'>
      <TitleBar />
      
      <div className='flex flex-1'>
        <div className='sidebar'>
          <div className='sidebar-header-container'>
            <h2 className='sidebar-header'>Practice Mode</h2>
          </div>

          <Accordion 
            data={accordionData} 
            onSelect={(item) => fetchQuestions({
              year: item.year,
              topic: item.topic,
              subtopic: item.subtopic
            })}
          />
        </div>

        <div className='flex-1 p-4'>
          <div className='portal-body'>
            <div className='results-area'>

              <div className='results-box'>
                {questions.length > 0 ? (
                    <div className='results-grid'>
                      {renderPools(pools)}
                    </div>
                ) : (
                  <div className='results-empty'>
                    <p className='results-empty-message'>Select a topic to begin practice</p>
                    <p className='results-empty-message'>If no results are displayed, you haven't yet completed questions of this subtopic :(</p>
                  </div>
                )}
              </div>
              <p className='results-hint'>
                Not seeing the result you want? Talk to your teacher or double check the name you are searching for :D
              </p>
            </div>
          </div>
        </div>
      </div>
      <Footer />
    </div>
  );
};

export default Practice;